...
#2.5a
n5 <- 5
mat_2.5a <- outer(0:(n5-1), 0:(n5-1), FUN = function(i,j) (i+j) %% 5)
#2.5b
n <- 10
mat_2.5b <- outer(0:9, 0:9, \(i,j) (i+j) %% 9)
#2.5c
n <- 9
mat_2.5c <- outer(0:(n-1), 0:(n-1), function(i, j) (i - j) %% 9)
...
#4.2a
getColsNoNA <- function(mat) mat[, colSums(is.na(mat)) == 0]

#4.2b
getSubmatrixNoNA <- function(mat) {
  r <- rowSums(is.na(mat)) == 0
  c <- colSums(is.na(mat)) == 0
  mat[r, c, drop = FALSE]
}

matA <- matrix(c(1, NA, 3, 4, 5, 6, NA, 8, 9), 3)
getColsNoNA(matA)
getSubmatrixNoNA(matA)
...
#5.3a
coeff <- 1:10
vec_5.3a <- sapply(myList, function(lst) sum(lst$x * coeff) / sum(lst$y * coeff))

#5.3b
mat_5.3b <- sapply(1:10, function(j) sapply(myList, function(lst) lst$x[j] - lst$y[j]))

#5.3c
numerator <- sum(sapply(1:1000, function(i) i * myList[[i]]$x[2]))
denominator <- sum(sapply(1:1000, function(i) i * myList[[i]]$y[2]))
val_5.3c <- numerator / denominator
